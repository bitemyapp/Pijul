...
val parse_url : string -> repository_ref

val default_remote_file : string
val get_remote : string -> string -> string option
val url : repository_ref -> string
...
val branch_patches : Mdb.txn -> repository -> string -> (string->unit) -> unit

val branch_revpatches : Mdb.txn -> repository -> string -> (string->unit) -> unit

val branch_previous_patch : Mdb.txn -> repository -> string -> string -> string

val branch_last_patch : Mdb.txn -> repository -> string -> string

val streq : string -> int -> string -> int -> int -> int
(** {2 Patches} *)
...
val json_input_patch : in_channel -> patch

val input_dependencies : in_channel -> string list

val edge_contents : Mdb.txn -> repository -> string -> string -> string

val is_deleted : char -> bool
val is_folder : char -> bool
val is_new : char -> bool
val line_size : int
val key_size : int
val path_of_inode : Mdb.cursor -> string -> string list
val path_of_folder_node : Mdb.txn -> repository -> string -> string list list
...
val record : working_copy:string -> Mdb.txn -> repository -> ((string*string) list) list * string M.t
...
val dependencies : (string*string) list list -> S.t
...
val rollback : Mdb.txn -> repository -> (string*string) list list -> string -> (string*string) list list
...
val output_repository :
  working_copy:string -> Mdb.env -> Mdb.txn -> repository -> (string*string) list list -> unit
...
val with_env : string -> (Mdb.env -> 'a) -> 'a
val with_txn : Mdb.env -> Mdb.txn option -> (Mdb.txn -> 'a) -> 'a
val with_cursor : Mdb.txn -> Mdb.dbi -> (Mdb.cursor -> 'a) -> 'a
...
val compare_repositories : string -> string -> string list

val apply_recursive : pijuldir:string -> Mdb.txn -> repository -> string -> unit
